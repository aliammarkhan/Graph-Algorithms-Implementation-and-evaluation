...
def main_Belman_Ford(filename):
    DG = nx.DiGraph()  # creating a Driected Graph
        ...
        while count < V: #relaxing every edge for V-1 times
            for (v, i) in Edges:
                ...
                if DG[v][i]['weight'] + distance[v] < distance[i]:
                    ...
                    in_loop = True
                # if DG[i][v]['weight'] + distance[i] < distance[v]: #uncomment if graph is undriected
                #   distance[v] = DG[i][v]['weight'] + distance[i]
                #  in_loop = True

            if not in_loop: # break if for a iteration no costs are changed
                break
            count += 1
            in_loop = False
